        ...
        event.getServer().ifPresent(server -> {
            event.getUser().ifPresent(user -> {
                ...
                ReactRoleMessage.loadReactRoleMessage(messageID).ifPresent(rrMsg -> {
                    rrMsg.getRoleIdByEmoji(userReaction).ifPresent(roleId -> {
                        server.getRoleById(roleId).ifPresent(role -> server.removeRoleFromUser(user, role));
                    });
                });
            });
        });
            ...
            event.getUser().ifPresent(user -> {
                ...
                ReactRoleMessage.loadReactRoleMessage(messageID).ifPresent(rrMsg -> {
                    rrMsg.getRoleIdByEmoji(userReaction).ifPresentOrElse(roleId -> {
                        server.getRoleById(roleId).ifPresent(role -> server.addRoleToUser(user, role));
