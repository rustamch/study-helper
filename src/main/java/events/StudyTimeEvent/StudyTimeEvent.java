...
public class StudyTimeEvent implements BotMessageEvent {
  ...
  public void invoke(MessageCreateEvent event, String[] content) {
    ...
    switch (command) {
      case "check":
        sendUserTimeForUser(event, content);
        break;
      case "leaderboard":
        processLeaderboardCommand(event, content);
        break;
      case "sub":
        subtractStudyTime(event, content);
        break;
    }
  }

  private void sendUserTimeForUser(MessageCreateEvent event, String[] content) {
    if (content.length > 1) {
  ...
  private void subtractStudyTime(MessageCreateEvent event, String[] content) {
    if (content.length > 1) {
      try {
  ...
  private void processLeaderboardCommand(MessageCreateEvent event, String[] content) {
    if (content.length > 1) {
